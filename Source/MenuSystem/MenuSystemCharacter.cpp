	...
	JoinSessionCompleteDelegate(FOnJoinSessionCompleteDelegate::CreateUObject(this, &ThisClass::OnJoinSessionComplete))
{
	...
	if(OnlineSubsystem)
	{
		...
		OnlineSessionInterface->OnCancelMatchmakingCompleteDelegates.AddUObject(this, &ThisClass::OnCreateSessionComplete);
		if(GEngine)
		{
...
void AMenuSystemCharacter::JoinGameSession()
{
	...
	UKismetSystemLibrary::PrintString(this, "BBBBBBBBBBBBBBBBBBBBBBBB");
}

void AMenuSystemCharacter::CreateGameSession()
{
	//Called enter pressing the 1 key
	if(!OnlineSessionInterface.IsValid())
	{
		return;
	}
...
void AMenuSystemCharacter::OnCreateSessionComplete(FName SessionName, bool bIsSuccessful)
{
	if(bIsSuccessful)
	{
	...
	{
		GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Red, FString::Printf(TEXT("Failied to create session")));
	}
}
...
void AMenuSystemCharacter::OnFindSessionComplete(bool bWasSuccessful)
{
	...
	if (bWasSuccessful)
	{
		UE_LOG(LogTemp, Display, TEXT("Found Session(s) number - %d"), SessionSearch->SearchResults.Num());
	}
...
void AMenuSystemCharacter::OnJoinSessionComplete(FName SessionName, EOnJoinSessionCompleteResult::Type Result)
{
	...
	if(GEngine)
	{
		GEngine->AddOnScreenDebugMessage(-1, 15.f, FColor::Green, FString::Printf(TEXT("Connect String: %s"), *Address));
	}
